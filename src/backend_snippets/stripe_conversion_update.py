...
def get_course_code(product_code):
    # This logic is kept for backward compatibility if needed,
    # but the primary flow now uses dynamic registration URLs.
    reg_form = None
    course_code = None
...
def lambda_handler(event, context):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        # Extract Payment Intent ID for correlation
        payment_intent = session.get("payment_intent")

        response = stripe.checkout.Session.list_line_items(session_id)
        product_code = response["data"][0]["price"]["product"]

        course_code, _ = get_course_code(product_code)

        email = session["customer_details"]["email"]
        name = session["customer_details"]["name"]

        # Construct the new dynamic registration URL
        # Uses the payment_intent as the unique ID
        reg_form_url = f"https://asyncacademy.ca/register?payment_id={payment_intent}"

        template_data = {
            "name": name,
            "course_code": course_code,
            "reg_form": reg_form_url
        }

        ses_client = boto3.client('sesv2')
        response = ses_client.send_email(
            FromEmailAddress='Asynchronous Academy <contact@asyncacademy.ca>',
